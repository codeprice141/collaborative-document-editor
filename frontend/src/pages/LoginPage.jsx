...
import { FileText, LogIn, Sun, Moon, Check, X, User } from "lucide-react";

const GOOGLE_ACCOUNTS = [
  { name: "Rishabh Dev", email: "rishabh.dev@gmail.com", avatarColor: "#2563eb" },
  { name: "Raj Sharma", email: "raj.sharma@gmail.com", avatarColor: "#16a34a" },
  { name: "Alex Johnson", email: "alex.tech@gmail.com", avatarColor: "#9333ea" },
];

const GITHUB_ACCOUNTS = [
  { name: "rishabh-dev", email: "rishabh.gh@github.com", avatarColor: "#0f172a" },
  { name: "raj-coder", email: "raj.coder@github.com", avatarColor: "#475569" },
];
...
export default function LoginPage() {
  ...
  const [oauthModalProvider, setOauthModalProvider] = useState(null); // 'Google' | 'GitHub' | null
  ...
  const handleSelectOAuthAccount = async (account) => {
    setLoading(true);
    setOauthModalProvider(null);
    try {
      try {
        await login(account.email, "oauth_secure_pass_123");
      } catch {
        await register(account.email, "oauth_secure_pass_123", account.name);
      }
      navigate("/dashboard");
    } catch (err) {
  ...
  const handleCustomOAuthSubmit = (e) => {
    e.preventDefault();
    if (!customOAuthEmail.trim()) return;
    const name = customOAuthEmail.split("@")[0].replace(".", " ");
    handleSelectOAuthAccount({
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "var(--bg-primary)", padding: "1rem", position: "relative" }}>
      ...
      {/* Realistic Google / GitHub Account Chooser Modal */}
      {oauthModalProvider && (
        <div style={{
          ...
          <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: "0.75rem", color: "var(--text-secondary)" }}>
                    to continue to CollabEditor
                  </p>
                </div>
              </div>
              <button onClick={() => setOauthModalProvider(null)} style={{ border: "none", background: "none", cursor: "pointer", color: "var(--text-secondary)" }}>
                <X size={19} />
              </button>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", marginBottom: "1.25rem" }}>
              {(oauthModalProvider === "Google" ? GOOGLE_ACCOUNTS : GITHUB_ACCOUNTS).map((acc) => (
                <div
                  key={acc.email}
                  onClick={() => handleSelectOAuthAccount(acc)}
                  style={{
                ...
                >
                  <div style={{
                    ...
                    backgroundColor: acc.avatarColor, color: "#ffffff",
            ...
            <form onSubmit={handleCustomOAuthSubmit} style={{ borderTop: "1px solid var(--border-color)", paddingTop: "1rem" }}>
              <div style={{ fontSize: "0.75rem", fontWeight: "600", color: "var(--text-secondary)", marginBottom: "0.4rem" }}>
                Use another {oauthModalProvider} account:
              </div>
              <div style={{ display: "flex", gap: "0.4rem" }}>
                <input
                  type="email"
                  placeholder={`your.name@${oauthModalProvider.toLowerCase()}.com`}
                  value={customOAuthEmail}
                  onChange={(e) => setCustomOAuthEmail(e.target.value)}
                  style={{ flex: 1, padding: "0.5rem 0.75rem", borderRadius: "8px", fontSize: "0.8125rem", outline: "none" }}
                />
                <button
                  type="submit"
                  style={{
                    padding: "0.5rem 0.85rem", backgroundColor: "#2563eb", color: "#ffffff",
